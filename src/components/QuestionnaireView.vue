...
const bmiInfo = computed(() => {
  ...
  if (v < 24) return { text: '体重正常', color: 'text-[#0B6BCB]', bg: 'bg-[#0B6BCB]/10', tip: '体型很标准！和营养师一起，练出更好的状态', range: '18.5 ~ 23.9' };
...
// 选项大按钮（有/无、是/否）
const yesNoOptions = [
  { label: '没有', value: '无', activeClass: 'bg-[#0B6BCB] text-white border-[#0B6BCB]' },
  { label: '有', value: '有', activeClass: 'bg-[#FF976A] text-white border-[#FF976A]' },
];
const ynOptions = [
  { label: '否', value: '否', activeClass: 'bg-[#0B6BCB] text-white border-[#0B6BCB]' },
  { label: '是', value: '是', activeClass: 'bg-[#FF976A] text-white border-[#FF976A]' },
];
const freqOptions = [
  { label: '从不', value: '从不', activeClass: 'bg-[#0B6BCB] text-white border-[#0B6BCB]' },
  ...
  <div class="flex min-h-full flex-col bg-[#F7F8FA] pb-safe relative">
    ...
    <div class="p-4 flex-1 flex flex-col space-y-4 pb-32">
      ...
      <div class="flex items-center justify-between px-1">
        ...
        <div class="text-xs font-bold text-[#0B6BCB]">{{ step }}/{{ steps.length }}</div>
      </div>
      ...
      <div class="mb-1">
        <div class="flex justify-between text-[10px] text-gray-400 mb-2">
          <div
            ...
            :key="s.key"
            :class="['flex-1 text-center transition-colors duration-300', step === idx + 1 ? 'font-bold text-[#0B6BCB]' : step > idx + 1 ? 'text-[#0B6BCB]' : '']"
          >
            {{ s.label }}
          </div>
        </div>
        <div class="h-1.5 bg-gray-200 rounded-full overflow-hidden">
          <div
            class="h-full bg-gradient-to-r from-[#0B6BCB] to-[#12B5C2] rounded-full transition-all duration-500 ease-out"
      ...
      <Card v-if="step === 1" key="s1" class="space-y-5 flex-1">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input type="text" placeholder="请输入真实姓名" class="w-full rounded-xl border border-gray-200 p-3.5 focus:border-[#0B6BCB] focus:outline-none focus:ring-2 focus:ring-[#0B6BCB]/10 transition-all" :value="formData.name" @input="formData.name = ($event.target as HTMLInputElement).value; error = ''" />
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <button
                ...
                :class="['py-4 rounded-2xl border-2 flex flex-col items-center gap-1.5 transition-all active:scale-95', formData.gender === 'male' ? 'bg-[#0B6BCB]/10 border-[#0B6BCB] text-[#0B6BCB]' : 'bg-white border-gray-200 text-gray-500']"
              >
          ...
          <div>
            ...
            <input type="number" inputmode="numeric" placeholder="例如: 32" class="w-full rounded-xl border border-gray-200 p-3.5 focus:border-[#0B6BCB] focus:outline-none focus:ring-2 focus:ring-[#0B6BCB]/10 transition-all" :value="formData.age" @input="formData.age = ($event.target as HTMLInputElement).value; error = ''" />
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="text-sm font-medium text-gray-700 block mb-1.5">身高 (cm) <span class="text-red-500">*</span></label>
              <input type="number" inputmode="decimal" placeholder="170" class="w-full rounded-xl border border-gray-200 p-3.5 focus:border-[#0B6BCB] focus:outline-none focus:ring-2 focus:ring-[#0B6BCB]/10 transition-all" :value="formData.height" @input="formData.height = ($event.target as HTMLInputElement).value; error = ''" />
            </div>
            <div>
              <label class="text-sm font-medium text-gray-700 block mb-1.5">体重 (kg) <span class="text-red-500">*</span></label>
              <input type="number" inputmode="decimal" placeholder="65.5" class="w-full rounded-xl border border-gray-200 p-3.5 focus:border-[#0B6BCB] focus:outline-none focus:ring-2 focus:ring-[#0B6BCB]/10 transition-all" :value="formData.weight" @input="formData.weight = ($event.target as HTMLInputElement).value; error = ''" />
            </div>
          </div>
            ...
            <div v-if="bmiInfo" class="rounded-2xl p-4 flex items-start gap-3 border border-gray-100" :class="bmiInfo.bg">
              <div class="flex-1">
                ...
                <div class="flex gap-1.5 text-[10px] flex-wrap">
                  ...
                  <span class="px-1.5 py-0.5 rounded bg-white/70" :class="bmiInfo.text === '体重正常' ? 'font-bold text-[#0B6BCB]' : 'text-gray-400'">正常 18.5~23.9</span>
        ...
        <div v-for="q in [
          ...
          <transition name="step-fade">
            <input
              ...
              class="w-full rounded-xl border border-gray-200 p-3.5 focus:border-[#0B6BCB] focus:outline-none focus:ring-2 focus:ring-[#0B6BCB]/10"
        ...
        <div class="grid grid-cols-2 gap-2">
          <div class="min-w-0">
            ...
            <button type="button" @click="openTimePicker('wakeTime')"
              class="w-full px-2 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#0B6BCB] outline-none flex items-center justify-between">
          ...
          <div class="flex items-center gap-3 bg-gray-50 rounded-2xl p-3">
            ...
            <div class="flex-1 text-center" @click="startEdit('sleepDuration')">
              <input
                ...
                @blur="stopEdit"
                v-focus
                class="w-24 text-3xl font-black text-gray-900 text-center bg-white rounded-lg border border-[#0B6BCB] focus:outline-none py-1"
              />
              <template v-else>
          ...
          <div class="flex items-center gap-3 bg-gray-50 rounded-2xl p-3">
            ...
            <div class="flex-1 text-center" @click="startEdit('exerciseFrequency')">
              <input
                ...
                @blur="stopEdit"
                v-focus
                class="w-24 text-3xl font-black text-[#0B6BCB] text-center bg-white rounded-lg border border-[#0B6BCB] focus:outline-none py-1"
              />
              <template v-else>
                <span class="text-3xl font-black text-[#0B6BCB]">{{ formData.exerciseFrequency }}</span>
        ...
        <div>
          ...
          <div class="grid grid-cols-3 gap-2.5">
            <button
              ...
              :class="['py-3.5 rounded-2xl border-2 flex flex-col items-center gap-1.5 transition-all active:scale-95', formData.exerciseTypes.includes(opt.label) || (opt.label === '其他' && formData.exerciseTypes.some((x) => !EXERCISE_OPTIONS.some((o) => o.label === x))) ? 'bg-[#0B6BCB]/10 border-[#0B6BCB] text-[#0B6BCB]' : 'bg-white border-gray-200 text-gray-500']"
            >
      ...
      <div v-else-if="step === 5" key="s5" class="space-y-5 flex-1">
        <!-- 健康画像摘要 -->
        <Card class="bg-gradient-to-br from-[#0B6BCB]/5 to-teal-50 border-[#0B6BCB]/20">
          <h3 class="font-bold text-gray-900 mb-3 flex items-center gap-2">
            <Sparkles class="w-4 h-4 text-[#0B6BCB]" />
            你的健康画像
          </h3>
        ...
        <Card class="space-y-4">
          ...
          <div class="grid grid-cols-3 gap-2">
            ...
            <button
              ...
              class="aspect-[3/4] flex flex-col items-center justify-center rounded-lg border border-dashed border-[#0B6BCB]/40 bg-[#0B6BCB]/5 text-[#0B6BCB] hover:bg-[#0B6BCB]/10 transition-colors"
            >
    ...
    <div class="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-sm border-t border-gray-100 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
      <div class="max-w-md mx-auto flex gap-3">
        ...
        <Button class="flex-1 bg-[#0B6BCB] hover:bg-[#0B6BCB]/90 text-white" @click="handleNext">
