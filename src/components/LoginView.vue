...
<template>
  <div v-if="step === 1" class="flex flex-col bg-gradient-to-b from-[#F7F8FA] to-white" style="min-height: 100vh; min-height: 100dvh;">
    ...
    <div class="flex flex-1 flex-col items-center justify-center p-6 space-y-10">
      <div class="flex flex-col items-center space-y-4 mt-4">
        <div class="flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-[#0B6BCB] to-[#12B5C2] shadow-lg shadow-[#0B6BCB]/30">
      ...
      <div class="w-full space-y-4">
        <div class="grid grid-cols-3 gap-3 mb-8">
          <button
            @click="role = 'student'"
            :class="['flex flex-col items-center justify-center py-4 rounded-2xl border-2 transition-all', role === 'student' ? 'border-[#0B6BCB] bg-[#0B6BCB]/5' : 'border-gray-100 bg-white hover:border-[#0B6BCB]/30']"
          >
            <div :class="['p-2 rounded-full mb-2', role === 'student' ? 'bg-[#0B6BCB] text-white' : 'bg-gray-100 text-gray-500']">
              <UserCircle class="w-6 h-6" />
            </div>
            <span :class="['text-sm font-bold', role === 'student' ? 'text-[#0B6BCB]' : 'text-gray-600']">学员</span>
          </button>
          <button
            @click="role = 'coach'"
  ...
  <div v-else class="flex flex-col bg-white" style="min-height: 100vh; min-height: 100dvh;">
    ...
    <div class="flex-1 flex flex-col px-6 pt-12 pb-6">
      ...
      <div class="mt-12">
        <Button class="w-full h-12 text-base font-medium rounded-full shadow-lg shadow-[#0B6BCB]/20" @click="handlePhoneSubmit">
          确认绑定
        </Button>
