  ...
  <Card class="p-4">
    <div class="flex items-center justify-between mb-3">
      ...
      <div v-if="weightStats" class="flex items-center gap-1 text-xs">
        <component
          ...
          :class="['w-3.5 h-3.5', weightStats.change < 0 ? 'text-[#0B6BCB]' : weightStats.change > 0 ? 'text-orange-500' : 'text-gray-400']"
        />
        <span :class="weightStats.change < 0 ? 'text-[#0B6BCB]' : weightStats.change > 0 ? 'text-orange-500' : 'text-gray-400'">
    ...
    <div class="overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <svg :viewBox="`0 0 ${consts.CW} ${consts.CH}`" class="w-full touch-none select-none"
        ...
        <!-- Line -->
        <path v-if="linePath" :d="linePath" fill="none" stroke="#0B6BCB" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" />
        ...
        <line v-if="activeIdx !== null && chartPoints[activeIdx]"
              ...
              stroke="#0B6BCB" stroke-width="1" stroke-dasharray="3 3" opacity="0.4" />

        <!-- Data points -->
        <circle v-for="(pt, i) in chartPoints" :key="`pt-${i}`"
                :cx="pt.x" :cy="pt.y"
                :r="activeIdx === i ? 5.5 : 3"
                :fill="activeIdx === i ? '#0B6BCB' : '#fff'"
                stroke="#0B6BCB" stroke-width="1.5"
    ...
    <div v-if="selectedPoint" class="mt-3 p-3 rounded-xl bg-[#0B6BCB]/5 border border-[#0B6BCB]/10 flex items-center justify-between">
      <div>
      ...
      <div class="text-right">
        ...
        <div class="text-sm font-bold text-[#0B6BCB]">{{ selectedPoint.weight }} kg</div>
      </div>
      <div v-if="selectedPoint.change !== null" class="text-right">
        <div class="text-[10px] text-gray-500 mb-0.5">较上次</div>
        <div :class="['text-sm font-bold', selectedPoint.change < 0 ? 'text-[#0B6BCB]' : selectedPoint.change > 0 ? 'text-orange-500' : 'text-gray-500']">
