  ...
  nextTick(() => {
    if (recordId) {
      ...
      if (el) {
        ...
        el.classList.add('ring-2', 'ring-[#0B6BCB]');
        setTimeout(() => el.classList.remove('ring-2', 'ring-[#0B6BCB]'), 2000);
      }
    }
...
<template>
  ...
  <div v-else class="flex min-h-full flex-col bg-[#F7F8FA] pb-safe relative font-sans">
    ...
    <div class="bg-white px-4 pt-4 border-b border-gray-200 space-y-4">
      <Card class="flex items-center justify-between p-4 bg-[#0B6BCB]/5 border-[#0B6BCB]/20">
      ...
      <div v-if="studentCamps.length >= 1" class="bg-white px-4 py-2.5 flex items-center justify-between rounded-xl border border-gray-100 mb-4">
        ...
        <button class="text-xs text-[#0B6BCB] border border-[#0B6BCB] px-2.5 py-1 rounded-full font-bold active:bg-green-50" @click="showCampPicker = true">
          切换
        </button>
      </div>
    </div>
              ...
              <div class="p-4 bg-gray-50/50">
                <div v-if="exerciseCommentingId === record.id" class="space-y-3">
                  ...
                  <div class="flex justify-end gap-2">
                    ...
                    <Button class="bg-[#0B6BCB] text-white" size="sm" @click="handleSaveExerciseComment(record.id)">保存</Button>
                  </div>
                </div>
                <div v-else-if="record.coachComment" class="relative group">
                  ...
                  <div class="flex items-center gap-2 mt-1">
                    <button @click="startExerciseComment(record)" class="text-xs text-[#0B6BCB]">编辑</button>
                    <span v-if="record.coachName" class="text-[10px] text-gray-400">批注人：{{ record.coachName }}</span>
                  </div>
                </div>
                <button v-else @click="startExerciseComment(record)" class="flex items-center gap-1 text-sm text-[#0B6BCB] font-medium">
                ...
                >
                  <div class="flex items-center justify-between">
                    ...
                    <div v-if="getPrevWeight(rec) !== null" class="text-right">
                      <div :class="['text-xs font-bold', (rec.weight - getPrevWeight(rec)!) < 0 ? 'text-[#0B6BCB]' : 'text-orange-500']">
                  ...
                  <div v-if="rec.dietitianComment" class="mt-2 ml-11">
                    <div class="flex items-center gap-2 mb-1">
                      <span class="text-xs font-bold text-[#0B6BCB]">营养师批注</span>
      ...
      <div class="p-4">
        ...
        <div class="space-y-2">
          <button
            ...
            :class="['w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all', !selectedCampId ? 'border-[#0B6BCB] bg-green-50 text-[#0B6BCB]' : 'border-gray-200 bg-white text-gray-700 active:bg-gray-50']"
          >
          ...
          <button
            ...
            :class="['w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all', selectedCampId === camp.id ? 'border-[#0B6BCB] bg-green-50 text-[#0B6BCB]' : 'border-gray-200 bg-white text-gray-700 active:bg-gray-50']"
          >
