    ...
    <div class="px-4 mt-3 space-y-2">
      <div class="relative">
        ...
        <input v-model="searchKeyword" type="text" placeholder="搜索学员姓名或手机号"
          class="w-full pl-9 pr-3 py-2.5 bg-white rounded-xl border border-gray-100 text-sm focus:outline-none focus:border-[#0B6BCB]/40 shadow-sm placeholder:text-gray-300" />
      </div>
      <div class="flex justify-between items-center">
        <span class="text-[11px] text-gray-400">{{ studentGroups.length }} 名学员待批注</span>
        <div class="flex gap-2">
          <button @click="expandAll" class="text-[11px] font-bold text-[#0B6BCB] bg-white border border-[#0B6BCB]/30 px-2.5 py-1 rounded-full active:bg-green-50">全部展开</button>
          <button @click="collapseAll" class="text-[11px] font-bold text-[#0B6BCB] bg-white border border-[#0B6BCB]/30 px-2.5 py-1 rounded-full active:bg-green-50">全部收起</button>
        </div>
      </div>
    </div>
      ...
      <template v-if="studentGroups.length > 0">
        <div v-for="group in pagedGroups" :key="group.studentId" class="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
          <button @click="toggleExpanded(group.studentId)"
            class="w-full flex items-center justify-between px-4 py-3 bg-[#0B6BCB]/5 active:bg-[#0B6BCB]/10 transition-colors">
            <div class="flex items-center gap-2">
              ...
              <span class="text-[10px] text-[#0B6BCB] bg-[#0B6BCB]/10 px-1.5 py-0.5 rounded-full font-bold shrink-0">{{ group.records.length }}条待批注</span>
            </div>
          ...
          <div v-show="isExpanded(group.studentId)" class="divide-y divide-gray-50">
            <div v-for="r in group.records" :key="r.id" @click="openRecord(group.studentId, r.id)"
              ...
              <div class="w-12 h-12 rounded-lg overflow-hidden shrink-0 bg-gray-100 flex items-center justify-center">
                ...
                <div v-else class="w-full h-full bg-[#0B6BCB]/8 flex items-center justify-center">
                  <Dumbbell class="w-5 h-5 text-[#0B6BCB]" />
                </div>
              </div>
              <!-- 内容 -->
              <div class="flex-1 min-w-0">
                <div class="flex justify-between items-center mb-0.5">
                  <div class="flex items-center gap-1.5">
                    <span class="text-[10px] px-1.5 py-0.5 rounded font-bold bg-[#0B6BCB]/10 text-[#0B6BCB]">运动</span>
