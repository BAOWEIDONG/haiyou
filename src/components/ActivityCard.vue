      ...
      <template v-if="mediaItems.length > 1">
        ...
        <div class="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 pointer-events-none">
          <div
            ...
            :key="idx"
            :class="['h-1.5 rounded-full transition-all', idx === currentIndex ? 'w-4 bg-[#0B6BCB] shadow-sm' : 'w-1.5 bg-white/70 backdrop-blur-sm']"
          />
        </div>
      </template>
    </div>
    ...
    <div class="p-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <template v-if="campLabels.length > 0">
          <span v-for="label in campLabels" :key="label" class="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-[#0B6BCB]/10 text-[#0B6BCB] shrink-0">{{ label }}</span>
        </template>
      ...
      <button
        ...
        class="flex items-center gap-0.5 text-xs text-[#0B6BCB] font-medium mb-3 active:scale-95 transition-transform"
      >
        {{ expanded ? '收起' : '展开全文' }}
