...
// ─── 报告健康解读（解读聊天窗口待处理） ───
const interpretationPendingCount = computed(() => store.getOpenInterpretations().length);
</script>
...
<template>
  <div class="flex min-h-[100dvh] flex-col pb-24 font-sans bg-gradient-to-b from-[#FFF6EE] to-[#FFFDFB]">
    ...
    <div class="flex-1 px-5 space-y-4 relative -mt-2">
      ...
      >
        <div class="h-10 w-10 rounded-xl bg-[#8B5CF6] flex items-center justify-center shrink-0">
          <MessageCircleQuestion class="h-5 w-5 text-white" />
        </div>
        <div class="flex-1 min-w-0">
          <div class="text-sm font-bold text-[#0B6BCB">{{ reportPendingCount + interpretationPendingCount }} 份报告待办</div>
          <div class="text-[11px] text-gray-500 mt-0.5">档案转录 {{ reportPendingCount }} 份 · 解读跟进 {{ interpretationPendingCount }} 份 · 点击进入</div>
        </div>
