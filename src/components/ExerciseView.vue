...
<template>
  <div ref="sheetRoot" class="flex min-h-full flex-col bg-[#F7F8FA] pb-8 font-sans">
    ...
    <div v-show="activeTab === 'checkin'" class="p-4 space-y-4 pb-32">
      <!-- 今日已达上限提示 -->
      <div v-if="hasReachedDailyLimit" class="rounded-xl bg-[#0B6BCB]/8 border border-[#0B6BCB]/20 p-3 flex items-center gap-2">
        <div class="w-8 h-8 rounded-full bg-[#0B6BCB]/15 flex items-center justify-center shrink-0">
          <Dumbbell class="w-4 h-4 text-[#0B6BCB]" />
        </div>
        <div class="flex-1">
          <div class="text-sm font-bold text-[#0B6BCB]">今日运动打卡已达上限</div>
          <div class="text-[11px] text-gray-500">每天最多 {{ MAX_DAILY_EXERCISE }} 次运动打卡，明天再来吧</div>
        </div>
        <button @click="activeTab = 'records'" class="text-xs font-bold text-[#0B6BCB] bg-white px-3 py-1.5 rounded-full active:scale-95 transition-transform shrink-0">查看记录</button>
      </div>
      ...
      <Card v-for="(activity, index) in activities" :key="activity.id" class="space-y-5 relative pt-8 shadow-sm" :class="{ 'opacity-50 pointer-events-none': hasReachedDailyLimit }">
        ...
        <div class="absolute top-0 left-0 bg-[#0B6BCB]/10 text-[#0B6BCB] px-3 py-1 rounded-br-lg font-bold text-xs">
          运动项 {{ index + 1 }}
        </div>
        ...
        <div class="space-y-3">
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :class="['px-4 py-2 rounded-full text-sm font-medium transition-all active:scale-95', activity.type === type ? 'bg-[#0B6BCB] text-white shadow-sm scale-105' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']"
            >
              {{ type }}
            </button>
          </div>
        ...
        <div class="space-y-2">
          ...
          <div class="flex gap-2">
            <button
              ...
              :class="['flex-1 py-2 rounded-lg text-sm font-bold transition-all active:scale-95', activity.duration === String(d) ? 'bg-[#0B6BCB] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']"
            >
              {{ d }}
            </button>
          </div>
      ...
      <button
        ...
        class="w-full py-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 flex items-center justify-center gap-2 hover:border-[#0B6BCB] hover:text-[#0B6BCB] transition-all font-medium bg-white active:scale-[0.98]"
      >
        <Plus class="w-5 h-5" />
        添加运动项
      </button>
      ...
      <Card class="space-y-4 shadow-sm">
        ...
        <div>
          <div class="flex gap-2 mb-3">
            <button
              ...
              :class="['flex-1 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-1.5 transition-all active:scale-95', mediaType === 'photo' ? 'bg-[#0B6BCB] text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200']"
            >
            ...
            <button
              ...
              :class="['flex-1 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-1.5 transition-all active:scale-95', mediaType === 'video' ? 'bg-[#0B6BCB] text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200']"
            >
              <Video class="w-4 h-4" />
              视频打卡
            </button>
        ...
        <div v-if="mediaType === 'video'">
          ...
          <div v-if="videoCompressing" class="aspect-video rounded-xl border-2 border-[#0B6BCB]/30 bg-[#0B6BCB]/5 flex flex-col items-center justify-center gap-3">
            <Loader class="h-8 w-8 text-[#0B6BCB] animate-spin" />
            <span class="text-sm text-[#0B6BCB] font-medium">正在处理视频 {{ compressProgress }}%</span>
          ...
          <div v-else class="grid grid-cols-3 gap-2">
            ...
            <div
              ...
              class="aspect-square rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-100 transition-colors active:scale-95"
            >
              <Video class="h-6 w-6 text-[#0B6BCB] mb-1" />
        ...
        <div class="space-y-2">
          ...
          <textarea
            class="w-full rounded-lg border border-gray-200 p-3 text-sm focus:outline-none focus:border-[#0B6BCB] bg-gray-50 focus:bg-white transition-colors"
            rows="3"
    ...
    <div v-show="activeTab === 'trend'" class="p-4 space-y-4">
        ...
        <div class="pt-3 border-t border-gray-50 space-y-2">
          ...
          <div class="grid grid-cols-4 gap-2 text-center">
            ...
            <div class="bg-[#0B6BCB]/5 rounded-lg py-2">
              <div class="text-sm font-bold text-[#0B6BCB]">{{ totalExerciseDuration }}</div>
      ...
      <div v-else class="text-center py-10 bg-white rounded-2xl border border-gray-100">
        <div class="w-16 h-16 mx-auto mb-3 rounded-full bg-[#0B6BCB]/10 flex items-center justify-center">
          <TrendingUp class="w-8 h-8 text-[#0B6BCB]" />
        </div>
    ...
    <div v-show="activeTab === 'records'" class="p-4 space-y-4">
      <div v-if="groupedHistory.length === 0" class="text-center py-10 bg-white rounded-2xl border border-gray-100 animate-pop-in">
        <div class="w-16 h-16 mx-auto mb-3 rounded-full bg-[#0B6BCB]/10 flex items-center justify-center">
          <Dumbbell class="w-8 h-8 text-[#0B6BCB]" />
        </div>
        <div class="text-sm font-bold text-gray-700 mb-1">还没有运动记录</div>
        <div class="text-xs text-gray-400 mb-4">完成今天的第一次运动打卡吧</div>
        <button @click="activeTab = 'checkin'; activities[0].duration = '30'" class="text-xs font-bold text-[#0B6BCB] bg-[#0B6BCB]/10 px-4 py-2 rounded-full active:scale-95 transition-transform">从 30 分钟开始 →</button>
      </div>
      <div v-else class="space-y-4">
        <div v-for="group in groupedHistory" :key="group.date" :id="`exercise-group-${group.date}`">
          ...
          >
            <div class="flex items-center gap-2">
              <span class="w-1 h-4 rounded-full bg-[#0B6BCB]"></span>
            ...
            <Card v-for="record in group.records" :key="record.id" class="p-0 overflow-hidden">
              <div class="p-4 border-b border-gray-50">
                <div class="flex justify-between items-center mb-3">
                  ...
                  <span class="text-[10px] px-2 py-0.5 rounded text-[#0B6BCB] bg-[#0B6BCB]/10 font-bold uppercase">
                    {{ record.type }}
                  </span>
                </div>
    ...
    <div v-show="activeTab === 'checkin' && !hasReachedDailyLimit" class="fixed bottom-0 left-0 right-0 z-40 px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 bg-gradient-to-t from-[#F7F8FA] via-[#F7F8FA]/95 to-transparent">
      <Button class="w-full shadow-lg shadow-[#0B6BCB]/30 active:scale-95 transition-transform" size="lg" @click="handleSubmit">
        完成打卡
      </Button>
    </div>
  </div>
